...
[^no-redis]: `slowapi` and `fastapi-limiter` were both considered. `slowapi` pulls in `limits` and is built around a decorator on a route function, but SQLAdmin registers `login` by handle (`app.post("/api/login")(login)` at [`main.py:149`](backend/app/main.py#L149)), so the decorator form does not fit without restructuring. `fastapi-limiter` requires Redis outright. Neither buys anything the 60 lines in `rate_limit.py` do not, and both add a dependency to an image whose whole point is being one container.
