...
- **`ExplainDiagramPanel`** ([`dock/ExplainDiagramPanel.ts:141-147`](frontend/src/dock/ExplainDiagramPanel.ts#L141)) — **OUT**. "Plan steps" is `initiallyOpen: false`, so the panel normally has exactly **one** open section, and `setResizable` is a documented no-op below two open sections ("No gutter appears with fewer than two open sections"). The option would buy nothing in the default state.
