...
**Prerequisites (cross-repo) — already satisfied.** The login dialog consumes two `@jimka/typescript-ui` credential components — `UsernameField` and an enhanced `PasswordField` (a `newPassword?: boolean` flag; the login default is exactly what we need) — plus the library's existing `WebStorageProxy` for preset persistence. These components **are already implemented, merged, and built** (library plan at [`typescript-ui/plans/implemented/credential-field-components.md`](/home/jika/typescript/typescript-ui/plans/implemented/credential-field-components.md); the app resolves them through its `file:` link to the library, so no relink is needed). The implementer should simply confirm the imports resolve (`UsernameField`/`PasswordField` from `@jimka/typescript-ui/component/input`) — there is **no** ordering gate on a separate repo. (This was a cross-repo dependency, expressed in prose rather than `depends-on` frontmatter, which is within-repo only; it is now closed.)
...
**Critical consequence of `min_size=0` — credentials must be probed explicitly.** With `min_size=0`, `asyncpg.create_pool(...)` opens **zero** connections eagerly (asyncpg's `Pool._initialize` pre-connects only `if self._minsize`), so it does **not** dial Postgres and does **not** raise on bad credentials or an unreachable host. Therefore `create_session` MUST force a real connection immediately after creating the pool — `async with pool.acquire() as c: await c.execute("SELECT 1")` — and, on any exception, close the pool and re-raise so `login`'s handler maps it to **401**. Without this probe, login would return **200 + Set-Cookie for invalid credentials**, and the failure would only surface later on the first route `acquire()` — as a **400** for a credential/catalog rejection (an `asyncpg.PostgresError`, caught by the global `_pg_error_handler`, main.py:88) or an unhandled **500** for an unreachable host (a bare `OSError`, which that handler does not catch) — either way contradicting the "bad/unreachable → 401" Expected Behaviour. (Do not "fix" this by raising `min_size` to 1; that would eagerly hold a connection per idle session, fighting the eviction goal. The explicit probe validates once at login while keeping idle pools at zero connections.)
...
async def create_session(dsn_parts: ConnParts) -> Session      # opens pool, probes SELECT 1 (validates creds/reachability), mints ids; closes pool + re-raises on failure
...
`login` body shape: `{"host": str, "port": int, "database": str, "username": str, "password": str, "connectionId"?: str}` (`connectionId` defaults to `"default"`). Success → `200 {"connectionId": str, "csrfToken": str, "username": str, "database": str}` and `Set-Cookie`. `whoami` returns the **same fields including `csrfToken`** (so a page reload recovers the token without re-login).
...
export function csrfHeader(): Record<string, string>;             // { "X-CSRF-Token": token } when set, else {} (never a null-valued key)
...
export class PresetStore {
    ...
    list(): Promise<ConnectionPreset[]>;            // proxy.read() in try/catch, sorted by name; [] on a corrupt blob (read() throws synchronously)
...
The dialog content container is a semantic form built with the `{ tag: "form" }` Component option — a `Panel({ tag: "form", ... })`. Fields, in order: a **preset picker** (`ComboBox` listing the merged preset names, plus a blank "— none —"), `TextField` host, `TextField` port, `TextField` database, `new UsernameField()` (username), `new PasswordField()` (password — the login default, no flag), **and a submit control placed *inside* the form** (see next paragraph). The credential components emit `<input>`s already carrying `autocomplete="username"`/`name="username"` and `autocomplete="current-password"`/`name="password"`, so the field attributes need no manual wiring.

**Why the submit control must live inside the form (load-bearing).** The library `Dialog` renders its footer `buttons` in a **separate region from `contentComponent`** (see [`promptQueryName.ts`](frontend/src/promptQueryName.ts): `buttons` vs `contentComponent`, and `show()` resolves on a footer-button click). A footer button is therefore **outside** the `{ tag: "form" }` element, so clicking it does **not** dispatch the form's native `submit` event — and browser password managers hook precisely that `submit` event to offer "save password." Relying on the Dialog's footer button (or on implicit Enter alone) would leave the save prompt not firing. So: give the Dialog **no footer login button** (`buttons: []`, `closeOnBackdrop: false`, non-dismissable) and put the **"Sign in" control inside the form** as a `Panel({ tag: "button" })` with a child `Text("Sign in")` for its visible label (a `Panel` is a container, not a text component) — a `<button>` with no explicit type defaults to `type="submit"` inside a form, so no attribute-setting is needed. Then both Enter-in-a-field and a click on the in-form button dispatch the native `submit`. Attach the handler with the **library event API, not a raw DOM listener**: `Event.addListener(form, "submit", (e: SubmitEvent) => { e.preventDefault(); void attemptLogin(); })` (import `Event` from `@jimka/typescript-ui/core`; this is the same seam used across the library, e.g. `Event.addListener(button, "click", …)`). Do **not** use `form.getElement().addEventListener` — `Component.getElement()` returns a branded-number `Handle`, not an `EventTarget`, so that neither typechecks nor runs. Note `"submit"` is not otherwise used in the library, so the manual-verify step (below) MUST confirm the listener fires and `preventDefault` suppresses the navigation. On success, dismiss the Dialog via its public `hide(result)` API (`dialog.hide("close")` — Dialog.ts) and resolve `showLoginDialog()` with the `Session`; on failure, show the inline error and leave the dialog open.

**Dialog mount/resolve flow (with `buttons: []`).** `showLoginDialog` returns a `Promise<Session>` it constructs itself. It calls `void dialog.show()` to mount/open the modal (voided — its promise is not the result source) but does **not** rely on `show()`'s resolution for the result (an empty button row means `show()` resolves only when `hide()` is called). The `Event.addListener(form,"submit",…)` handler is the sole driver: on a successful `api.login(...)` it stores the `Session`, calls `dialog.hide("close")`, and resolves the outer promise; a failed login keeps the dialog open. (Do not `await dialog.show()` as the result source — that would deadlock with no buttons.)

**"No DOM manipulation" — the precise seam.** The ban is on *building* the credential inputs by hand — no `document.createElement("form")`, no `setAttribute("autocomplete"/"name")` on the `<input>` nodes (the `{ tag: "form" }` option and the credential components own those). *Listening* for the form's `submit` event via the library's `Event.addListener(form, "submit", …)` and focusing a field via the component's `.focus()` are permitted and expected — listening/focusing through the library API is not manipulating the credential markup. (Do not reach for `getElement().addEventListener`; `getElement()` returns a `Handle`, not an `EventTarget`.)
...
2. **`backend/app/connections.py`** — rewrite. Delete `pools`, `connection_dsns`, `open_pools`, `close_pools`, `get_pool`. Keep `_init_connection`. Add the `Session` dataclass, `_sessions` dict, and the functions in *Public API*. Constants at top with the CODE_CONVENTIONS magic-number comments: `SESSION_POOL_MAX_SIZE = 5`, `SESSION_IDLE_TIMEOUT_SECONDS = 1800`, `SWEEP_INTERVAL_SECONDS = 60`. Use `secrets.token_urlsafe(32)` for both ids. `create_session` must open the pool with `min_size=0, max_size=SESSION_POOL_MAX_SIZE` **and then probe** — `async with pool.acquire() as c: await c.execute("SELECT 1")` — to force a real connection that validates credentials/reachability (see the pool-eviction decision); on any exception, `await pool.close()` and re-raise so `login` maps it to 401. `session_pool_for` raises `NotFound` if `connection_id != session.connection_id`. Check: `grep -rn "get_pool\|open_pools\|DATABASE_URL" backend/app/` — after this rewrite the only remaining matches are in `backend/app/main.py` (updated in **step 5**); `auth.py`/`config.py` (steps 3–4) contain none of these symbols, so `main.py` hits here are expected, not a failure.

3. **`backend/app/auth.py`** — new file. `allowed_hosts()` parses `SQLADMIN_ALLOWED_HOSTS` (comma-split, strip, lowercase; empty → empty set). `is_host_allowed(host, port)` checks `f"{host}:{port}"` and bare `host` membership. `require_session` reads `request.cookies.get(SESSION_COOKIE_NAME)`, calls `get_session` (raises `Unauthorized`), bumps `last_seen = time.monotonic()`, returns the session. `require_csrf` compares `request.headers.get("X-CSRF-Token")` to `session.csrf_token`, raises `Forbidden` on mismatch/None. `login`: validate body keys (raise `ValidationError` on missing); `is_host_allowed` gate (raise `Forbidden`); `try: session = await create_session(...)` mapping `asyncpg.InvalidAuthorizationSpecificationError`/`InvalidPasswordError` → `Unauthorized("Invalid credentials")`, `asyncpg.InvalidCatalogNameError` (nonexistent target database) → `Unauthorized("Cannot open target database")`, and `OSError`/`ConnectionError`/`asyncpg.CannotConnectNowError`/`socket.gaierror` → `Unauthorized("Cannot reach database")` (do **not** leak the exception text verbatim; note `InvalidCatalogNameError` is a `PostgresError`, so map it **here** — otherwise the global `_pg_error_handler` would turn it into a 400); on success `response.set_cookie(SESSION_COOKIE_NAME, session.id, httponly=True, secure=True, samesite="lax", path="/")` and return the JSON body. `logout`: read cookie, `close_session`, `response.delete_cookie(...)`. Constant `SESSION_COOKIE_NAME = "sqladmin_session"`. Check: `poetry run python -c "import app.auth"`.
...
7. **`frontend/src/data/api.ts`** — add module-level `_csrfToken`, `setCsrfToken`, `csrfHeader()`. **`csrfHeader()` MUST return `{}` when no token is set** (and `{ "X-CSRF-Token": token }` once set) — never a key with a `null`/`undefined` value. This is load-bearing: `postJson` merges `csrfHeader()` into its headers, and the **existing** `frontend/src/data/api.test.ts` asserts (`:122`, `:154`) that `runQuery`/`runExplain` call `fetch` with headers deep-equal to exactly `{ "Content-Type": "application/json" }`; returning `{}` when unauthenticated keeps that invariant true, whereas an `{ "X-CSRF-Token": null }` merge would break both tests under `npx vitest run`. Merge `csrfHeader()` into `postJson`'s headers. Add `login`/`logout`/`whoami` (whoami maps a 401 to `null`, not a throw) and `getConfig()` (`GET /api/config` via `getJson`, no auth). Define `LoginDetails`/`Session`/`AppConfig`/`ConnectionPreset` types (or import from a new `frontend/src/contract` addition — mirror existing contract style). Check: `grep -n "X-CSRF-Token\|/api/config" frontend/src/data/api.ts`.

7a. **`frontend/src/data/api.test.ts`** (existing — extend, do not rewrite): the two current assertions that `postJson` sends only `{ "Content-Type": "application/json" }` must stay green (they now also prove `csrfHeader()` returns `{}` pre-login). Add one case: after `setCsrfToken("t")`, a `postJson` call sends headers including `"X-CSRF-Token": "t"`. Check: `cd frontend && npx vitest run api`.
...
9. **`frontend/src/data/presetStore.ts`** — new thin domain wrapper over the library `WebStorageProxy` (import `WebStorageProxy`, `Model`, `ModelRecord` from `@jimka/typescript-ui/data`). Module-level `PRESETS_KEY = "sqladmin.presets"` (flat, not per-connection — see the presets decision) and `PRESET_MODEL = new Model([{ name: "name" }, { name: "host" }, { name: "port" }, { name: "database" }], "name")` (primary key `name`). Constructor defaults `this._proxy = new WebStorageProxy({ key: PRESETS_KEY, storage: "local" })` but accepts an injected `WebStorageProxy` (for tests). `async list()`: `(await this._proxy.read()) as ConnectionPreset[]` sorted by `name`, guarded so a corrupt blob resolves to `[]`. **Use `try { … } catch { return [] }`, not a `.catch()` chain** — `WebStorageProxy.read()` is `Promise.resolve(this.readAll())` and `readAll()` calls `JSON.parse` unguarded, so a corrupt blob throws **synchronously** during the `read()` call; a `.catch()` on the returned promise would never see it. The same synchronous throw applies to `save`/`remove` (their internal `readAll()`), so wrap those reads in `try/catch` too, treating a corrupt existing blob as empty (so a save proceeds to create). `async save(preset)`: read the array; if an entry with that `name` exists → `await this._proxy.update(new ModelRecord(PRESET_MODEL, preset))`, else `await this._proxy.create(new ModelRecord(PRESET_MODEL, preset))`. `async remove(name)`: `await this._proxy.destroy(new ModelRecord(PRESET_MODEL, { name }))`. **Store only `{ name, host, port, database }`** — neither the type nor the `Model` has a credential field, so nothing can leak. Do NOT hand-roll `window.localStorage`. Check: `grep -n "password\|username\|window.localStorage" frontend/src/data/presetStore.ts` — expect zero.

10. **`frontend/src/shell/loginDialog.ts`** — new file, modeled on `promptQueryName.ts`. **Depends on the library credential components (cross-repo prerequisite — see Overview).** First `const config = await getConfig().catch(() => ({ presets: [], allowUserPresets: true }))`. Build the merged preset list: server presets (from `config.presets`, tagged origin `"server"`) plus, **only when `config.allowUserPresets`**, `await new PresetStore().list()` (tagged `"user"`). Build a `Dialog` (`closeOnBackdrop: false`, **`buttons: []` — no footer login button**, no Cancel; non-dismissable). **The content container MUST be a semantic form via the `{ tag: "form" }` Component option** — `Panel({ tag: "form", ... })` — and the **submit control MUST live inside that form** (a `Panel({ tag: "button" })` holding a child `Text("Sign in")` for the label; a `<button>` with no explicit type defaults to `type="submit"`). This is because `Dialog` renders footer `buttons` outside `contentComponent`, so only an in-form submit fires the native `submit` event a password manager hooks (see the Internal Structure "Why the submit control must live inside the form" note). Fields: a preset `ComboBox` (merged names, labelled by origin, + a blank entry), `TextField`s host/database and a port `TextField` (defaults host `localhost`, port `5432`), then `new UsernameField()` and `new PasswordField()` — the credential components supply the `autocomplete`/`name` attributes, so do **not** hand-set any input attributes and do **not** build a raw `<form>` via `document.createElement`. Wire login on the form's submit with the library event API: `Event.addListener(form, "submit", (e: SubmitEvent) => { e.preventDefault(); void attemptLogin(); })` (import `Event` from `@jimka/typescript-ui/core`). Do **not** use `getElement().addEventListener` — `getElement()` returns a branded-number `Handle`, not an `EventTarget` (won't typecheck). `showLoginDialog` builds its own `Promise<Session>`, calls `void dialog.show()` to mount, and lets the submit handler drive the outcome (see the Internal Structure "Dialog mount/resolve flow" note) — do **not** `await dialog.show()` as the result source (empty `buttons` → it only resolves on `hide()`). Preset controls, **rendered only when `config.allowUserPresets` is true**: **Save preset** (prompt for a name, then `await store.save({ name, host, port, database })` from the current field values — never credentials) and **Delete**, enabled only when the selected entry's origin is `"user"` (`await store.remove(selected)`, refresh the ComboBox) — never for a server preset. Selecting any preset fills host/port/database and `.focus()`es the username field. `attemptLogin()` calls `api.login(...)`; on rejection show the error `detail` inline (a muted `Text` line) and keep the dialog open; on success call `dialog.hide("close")` (the public Dialog close API — Dialog.ts) and resolve `showLoginDialog()` with the `Session`. Check: `grep -n 'tag: *"form"\|tag: *"button"\|UsernameField\|PasswordField\|allowUserPresets\|getConfig\|Event.addListener(.*"submit"' frontend/src/shell/loginDialog.ts` — expect the form tag, the in-form submit button, both credential components, the gating, and the `Event.addListener` submit listener present; `grep -n 'createElement\|setAttribute\|getElement().addEventListener' frontend/src/shell/loginDialog.ts` — expect zero (no DOM hacks).
...
15. **`backend/tests/test_auth.py`** — new test file covering the auth/CSRF/allowlist behaviour from *Expected Behaviour*. Pure-logic units (no app): `is_host_allowed` (exact `host` and `host:port` membership; empty allowlist → `False`; supplied host lowercased before matching) and `session_pool_for` (returns pool on match, `NotFound` on mismatch). Route-level units via httpx `ASGITransport(app=app)` + `AsyncClient` in the `pytest-asyncio` auto style (no real Postgres — these paths reject before dialing): login with a host absent from `SQLADMIN_ALLOWED_HOSTS` → **403**; login with an empty allowlist → **403**; login to an allowed but non-listening `host:port` → **401** with a generic detail that never contains the password or raw asyncpg text; a protected route with no cookie → **401**; a mutating route with a stubbed session (via `app.dependency_overrides[require_session]`) but no `X-CSRF-Token` → **403**; `sweep_idle_sessions` drops a `Session` inserted with an old `last_seen`. Check: `poetry run python -m pytest tests/test_auth.py`.

16. **`backend/README.md`** — update the now-stale docs (see *Documentation Impact*). Replace the `DATABASE_URL … uvicorn` run command with one that sets `SQLADMIN_ALLOWED_HOSTS` (required — default-deny means an unset allowlist rejects every login) and, optionally, `SERVER_PRESETS`/`ALLOW_USER_PRESETS`; drop the `DATABASE_URL` line entirely. Fix the header blurb ("`connectionId -> asyncpg.Pool` registry") and the Layout lines for `main.py` ("lifespan (pool open/close)") and `connections.py` ("pool registry") to describe the per-session model (login-created session pools + a background idle sweep; no startup pool). Check: `grep -n "DATABASE_URL\|pool open/close\|pool registry" backend/README.md` — expect zero.
...
| Modify | `frontend/src/data/api.test.ts` (existing) |
...
- **Login, allowed host but bad credentials / nonexistent database / unreachable DB** → **401** with a generic detail (`"Invalid credentials"`, `"Cannot open target database"`, or `"Cannot reach database"`), no cookie, and the detail never contains the password or raw asyncpg text. (Testable against an allowed but non-listening `host:port` → the unreachable path; credential rejection and the nonexistent-database (`InvalidCatalogNameError`) path need a real Postgres → **manual verify**.)
...
- **Corrupt value** → a non-JSON blob under the key yields `[]` from `list()` (a `try/catch` guards `WebStorageProxy.read()`'s **synchronous** `JSON.parse` throw — not a `.catch()` chain), never throws.
...
- **Frontend unit tests:** `cd frontend && npx vitest run` — the new `presetStore.test.ts` covers the `PresetStore` cases above (over `WebStorageProxy` with a stubbed `Storage` global, per the library's `WebStorageProxy.test.ts`), and the **existing** `frontend/src/data/api.test.ts` (extended in step 7a) must stay green: its `postJson` header assertions (`api.test.ts:122,154`) pin the no-token request headers to exactly `{ "Content-Type": "application/json" }`, which the `csrfHeader() → {}`-when-unset invariant preserves, plus the new token-set case.
...
- [`backend/README.md`](backend/README.md) is the only doc that describes the removed architecture and must be updated (step 16). Specifically:
  - **Header blurb** (README line 3-5) calls the backend a "`connectionId -> asyncpg.Pool` registry" — now per-session pools created at login, not a registry seeded at startup.
  - **Run command** (README lines 11-12) sets `DATABASE_URL=…` and runs uvicorn. `DATABASE_URL` is **deleted** by this plan, and default-deny means an unset `SQLADMIN_ALLOWED_HOSTS` rejects every login — so the documented command no longer works. Replace with `SQLADMIN_ALLOWED_HOSTS=localhost:5432 poetry run uvicorn app.main:app --reload --port 8000`, and mention the optional `SERVER_PRESETS` / `ALLOW_USER_PRESETS` env vars.
  - **Layout** (README lines 25, 29) says `main.py` does "lifespan (pool open/close)" and `connections.py` is a "pool registry" — reword to the per-session session store + background idle sweep, with no startup pool.
- No other doc references the pool registry / `DATABASE_URL` (`grep -rn "DATABASE_URL\|pool registry" --include=*.md .` — expect only `backend/README.md` before the change, zero after). No public docs site or API-reference catalog covers the backend, so there is nothing else to update.
...
- **Breaking credential-manager save/fill — two ways to get it wrong.** With the credential components owning the field `autocomplete`/`name` attributes, the save/fill still breaks if either (a) the content container is left the default `div` instead of `{ tag: "form" }`, or (b) the submit control sits in the Dialog **footer** instead of **inside** the form — in both cases no native `submit` event reaches the password manager (see step 10 and the Internal Structure note). Verify in the DOM that the dialog content is a real `<form>`, that the "Sign in" button is a `<button>` **inside** that form, and that the components rendered `<input autocomplete="username">` / `<input autocomplete="current-password">`. Then confirm the browser actually prompts to save on a successful sign-in. Do **not** hand-set the input attributes or build a raw `<form>` via `document.createElement` — that is the library components' and the `tag` option's job now.
- **Cross-repo prerequisite — closed.** The login-dialog step imports `UsernameField`/`PasswordField` from `@jimka/typescript-ui`. These are already implemented, merged, and built (see Overview → Prerequisites), and the app resolves them via its `file:` link, so the import compiles today — no ordering gate remains. If a `tsc`/import error nonetheless appears, re-run the library build so `dist/` is fresh, rather than treating it as an unimplemented dependency.
...
- The library credential components `UsernameField` and enhanced `PasswordField` (`@jimka/typescript-ui/component/input`) — consumed by the login dialog for the `autocomplete`/`name` attributes. **Already merged and built** (`UsernameField.ts`, `PasswordField.ts` with the `newPassword?` flag, both exported from `component/input/index.ts`); the app resolves them through its `file:` link.
- [`typescript-ui/plans/implemented/credential-field-components.md`](/home/jika/typescript/typescript-ui/plans/implemented/credential-field-components.md) — the (now-implemented) library plan that defined `UsernameField`/`PasswordField`, for reference only. The dependency is closed; no ordering gate remains.
