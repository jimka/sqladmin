...
- The editor is `new TextArea()`; the seed is applied with **`editor.setValue(initialSql)`**, NOT the positional constructor text. `new TextArea(text)`'s positional `text` lands in `_defaultOptions` (class defaults), which `getValue()`/`render()` never consult (they read the instance bag `_options.text`), so it is silently dropped — `setValue` writes `_options.text`. (This is a real library bug in `TextArea`'s constructor: per-instance text belongs in the options bag, not the defaults bag; the app sidesteps it by using the setter, the documented runtime path.)
