...
Wiring the editor's Ctrl/Cmd+Enter requires listening to the `TextArea`'s `keydown`. ARCHITECTURE.md reserves the raw `Event` API for *self* (`Event.addListener(this, …)` from inside the owning component) and forbids a consumer reaching into another component's event routing; it prescribes instead *widening that component's typed `on()` surface*. `TextInput` exposed only `action`/`change`/`binding`, and the app cannot subclass the callable `TextArea`/`Component` exports to own the listener (the built `.d.ts` callable type drops instance methods — the same constraint that makes `SqlAdminShell` a factory). So this feature adds **one small additive library change**: a `keydown` shorthand on `TextInput.on()`/`off()`, routing to `Event.addListener(this, "keydown", …)` exactly as the existing `action` shorthand routes to `input`. The app then wires `editor.on("keydown", …)` — fully compliant. This is additive (a new overload) and breaks nothing. (A second, separate library change this feature needs is the `TextArea` constructor fix below.)

### `new TextArea(text)` must apply the positional text (library bug fix)

Seeding the editor with the generated `SELECT` relies on `new TextArea(initialSql)`. That was broken: `TextArea`'s constructor passed the positional `text` as a *subclass default* (into `_defaultOptions`, which holds class-level defaults), but the value getters (`getText`/`getValue`) and `render()` read the instance bag `_options.text` — which `applyOptions` only populates from the *options* argument. So the positional text was silently dropped (`new TextArea("x").getValue() === ""`) and never rendered. The fix passes the positional `text` through the **options** bag instead (an explicit `options.text` still wins), so `applyOptions` dispatches `setText` into `_options`. This is a real bug affecting every consumer, fixed on a small library branch; the app then seeds with `new TextArea(initialSql)` directly.

---

## Public API

**Two library changes:** (1) `TextInput.on("keydown", listener)` / `off("keydown", listener)` — an additive typed shorthand over the native `keydown` DOM event; (2) a `TextArea` constructor fix so `new TextArea(text)` applies the positional `text` (it was dropped into the defaults bag). Both are described in the architecture decisions above. Everything else is already published: `TextArea`, `Table`, `MemoryStore`, `Dock.addPanel`/`addLazyPanel`, `Button`, `ToolBar`, `Split`/`Border`/`Fit`.
...
└─ CENTER : body (Component, vertical Split):
            • before any run / after a non-row statement:
                one pane → TextArea fills the body (no result pane, no gutter)
            • after a rows result:
                two panes → TextArea (~150px) ── draggable gutter ── result
                            Table(MemoryStore, { columns: [] }) fills the rest
```

- The editor is `new TextArea(initialSql)` — the positional constructor text, which now applies the seed after the `TextArea` constructor fix above (before the fix it was dropped; the app briefly used `setValue` as a workaround).
- **The body is a vertical `Split`.** It starts with the editor as the sole pane (fills the body, no gutter). On the first rows result the result pane (`Panel(Fit)` holding the `Table`) is added — a draggable gutter appears — and the editor is seeded to **150px** via `split.setPaneSize(editor, 150)` / `setPaneSize(result, innerHeight - 150)`, after which the user can drag to resize. `setPaneSize` is px, so the body's `getInnerSize().height` supplies the total; on the "Open as query" auto-run that height isn't known yet (the panel runs before its first rAF-scheduled layout), so the seeding **retries on the next animation frame until the body is measured** (capped, and only while the panel still has an element). Subsequent rows runs reuse the pane (only the `Table` swaps), preserving the user's gutter position; a non-row statement removes the result pane so the editor fills again.
- Run handler: read SQL → if blank, `notify("Enter a SQL statement")` and return; else disable Run, `notify("Running…")`, `await runQuery(sql)`; on `kind:"rows"` build a fresh `MemoryStore` from `buildQueryModel(result.columns)` + `result.rows` and show the result pane with `Table(...)`, then `notify(\`${result.rowCount} row(s)\`)`; on `kind:"status"` hide the result pane and `notify(result.command)`; on throw, call the injected `onError(err)` (= `controller.notifyError`) — re-enable Run in a `finally`.
- **Seeded open:** when `opts.autoRun` is set (the "Open as query" path), the panel invokes the run handler once; `editor.getValue()` returns the constructor seed so the generated `SELECT` shows in the editor and its rows render immediately.
...
10. `frontend/src/dock/QueryPanel.ts`: the callable factory per *Internal Structure* — `new TextArea(initialSql)` (positional seed; works after the library `TextArea` constructor fix) + Run toolbar; the body is a vertical `Split` that holds the editor alone (fills) until a rows result adds the result pane (draggable gutter) and seeds the editor to 150px via `setPaneSize`, retrying on rAF until the body is measured (for the auto-run case); Ctrl/Cmd+Enter run; run-state disable; `autoRun` seeded-run; `onError` wiring. `glyphButton` is **module-private** in `TableWorkPanel.ts` ([`:162`](#)) — copy/re-implement it here (it cannot be imported); reuse the `Notify` type by copying its one-line definition (also module-local). Verify: `tsc` clean.
...
- **Before any run, the editor fills the whole panel and no result pane (or gutter) is shown** (the Split has a single pane). After a rows result, the editor drops to ~150px at the top with a draggable gutter and the grid fills the rest; the user can resize from there.
...
- **Library changes beyond these two** — the `@jimka/typescript-ui` changes are the additive `TextInput.on("keydown")` shorthand and the `TextArea` positional-constructor-text bug fix (both above); everything else composes already-published components.
