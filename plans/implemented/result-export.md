...
Both surfaces share **one pure serialization core** (`frontend/src/data/serialize.ts`) that turns `(columns, rows)` of **wire scalars** into a CSV or JSON string. The frontend rows arriving from `runQuery` ([`api.ts:86`](../frontend/src/data/api.ts#L86)) and from the row endpoints are already **contract `WireType` scalars** (the backend's `to_wire_value` maps every native asyncpg value before it leaves — [`wire.py:114`](../backend/app/wire.py#L114)), so the serializer operates directly on those scalars and never re-formats through the grid's cell renderers. The backend export reuses the *same* dialect rules in Python so a full-table CSV is byte-compatible with a query-result CSV of the same data **for every wire type except floating-point `number` values** — the query rows have already crossed JSON transport and arrive as JS numbers (`1.0` → `1`, `1e16` → `10000000000000000`), whereas the streamed export sees the native asyncpg float (`str(1.0)` → `1.0`, `1e+16`). That divergence is inherent to the query path, not the serializer; string-based types (numeric/decimal precision strings, ISO timestamps, base64 bytea, booleans, non-float json) stay byte-identical. See *Potential Challenges*.
...
- The same CSV/JSON cases as above, asserted in `pytest` against `csv_header`/`csv_row`/`json_row` — so a full-table CSV is byte-identical to a query CSV of the same wire data (NULL vs empty, quoting, each string-based wire type, non-ASCII json as raw UTF-8, header). The one exception is floating-point `number` values (see *Potential Challenges*): the frontend's query rows are already JS numbers, so a `1.0` renders `1` there but `1.0` in the native-float backend export — the tests exercise integer numbers, where both sides agree.
...
- **Float `number` byte-identity is unachievable across the two CSV surfaces** — the query-result export serializes JS numbers (the rows have already crossed JSON transport: `1.0` collapses to `1`, `1e16` widens to `10000000000000000`), while the full-table export serializes native asyncpg floats (`str(1.0)` → `1.0`, `str(1e16)` → `1e+16`). Matching them would require re-implementing ECMAScript number-formatting in Python *and* wiring every `number` as a string end-to-end (breaking the query grid) — disproportionate, and it still couldn't recover the fidelity the frontend already lost. So the documented byte-identity is narrowed to every wire type **except** floating-point `number`, and the full-table export is called out as the authoritative full-fidelity surface. `numeric`/`decimal`/`money` are unaffected (they already travel as precision strings). Non-float `json`/`jsonb` also stays byte-identical once both sides serialize with raw UTF-8 (`ensure_ascii=False` on the Python side), though a float *inside* a json value shares the same float caveat.
