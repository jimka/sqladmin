...
5. **Regression check:** `grep -n "setPreferredSize" frontend/src/shell/ActivityBar.ts` — expect two matches (the pre-existing `rail.setPreferredSize` and the bar's initial `activityBar.setPreferredSize` seed); the collapse-time call inside `setCollapsed` must be gone.
...
6. **Expand restores remembered width, stable across cycles:** after dragging the sidebar to, say, 340px then collapsing and re-expanding, the sidebar reopens to ~340px (not the default 280); a fresh session with no drag reopens to 280. Repeated collapse/expand cycles must hold the width — no progressive shrink. (Expand must set the dock's size too, not just the sidebar's: collapse inflates the dock, so setting only the sidebar leaves Σ overshooting `available` and the Split's proportional refill shrinks the sidebar each cycle — see LIBRARY_NOTES.md.)
...
- **Collapse pin depends on the hidden deck dropping out of the sidebar's inner min-width.** The pin holds at RAIL_WIDTH only if the sidebar's own `getMinSize` collapses to the rail width once the deck is hidden — i.e. `deck.setDisplayed(false)` must remove the deck from the inner Border's min-width aggregate (displayed-layout participation). If a hidden deck still contributed its min-width, the sidebar's effective min would exceed 40 and the pin would land wide. This is not offline-testable in sqladmin — watch it during live smoke behaviours 4 and 7 (collapse to exactly 40), and log to LIBRARY_NOTES.md if a workaround proves necessary.
