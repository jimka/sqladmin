...
**A confirmatory phase-2-style sweep, run separately at the user's request, widened finding (1) above.** Since `typescript-ui` 0.4.1 is deliberately not being published until after this branch merges (the whole point of running Phase 1's verification against the symlink first), the user asked for the M1 leak measurement to be re-confirmed against the current symlinked build before merging. It re-passed exactly: `wide.cols_20`, 4 cycles, flat at 532 rules after every close, zero growth. While re-running it, the same `DOM handle <n> is not registered` error from finding (1) reproduced on a **plain table tab close** — no query tab, no `CodeEditor`, no `keydown` involved at all. Root-caused to the library's own `Table` component: `Body.ts` and `Header.ts` register `click`/`contextmenu` subtree listeners the same way `QueryPanel.ts`'s editor registers its `keydown` one, and the tab-closing click is itself the event whose subtree walk trips over the just-released handle. This means the defect is not query-tab-specific or `keydown`-specific — it reproduces on the single most common tab type in the app, from library-internal code no app-side change can reach. `LIBRARY_NOTES.md`'s entry was broadened accordingly (title and body), still filed as one entry since the root cause is identical. Still out of this plan's scope for the same reasons as before; flagged here because its severity and reach are materially different from what was recorded at the time this plan's own audit converged. **Resolution:** the user decided this does not block adopting `0.4.1` — it does not stop the app from running, and it is scheduled for a `0.4.2` library release rather than delaying this one. `LIBRARY_NOTES.md`'s entry was updated to say so explicitly.

**Internal Phase 2 (steps 26-31) is now complete.** `typescript-ui` published `0.4.1` to the registry (`npm view @jimka/typescript-ui version` → `0.4.1`), satisfying the gate. The user performed the manifest bump and lockfile regen by hand (`frontend/package.json`'s range → `^0.4.1`, `npm install`, `frontend/node_modules/.vite` cleared), bundled into their own commit alongside SQLAdmin's own `0.3.0` → `0.4.0` version bump. Confirmed the three checks from step 29: `frontend/node_modules/@jimka/typescript-ui` is a real directory (not a symlink), its `package.json` reports `0.4.1`, and `package-lock.json`'s entry resolves the `typescript-ui-0.4.1.tgz` tarball.

**M1-M3 re-ran against the published tarball itself (no symlink)**, per the plan's phase-2 verification column:

- **M1** (`wide.cols_20`, 4 cycles) — exact pass, flat at 524 rules after every close, zero growth.
- **M2** (never-run query tab, 4 cycles) — exact pass via the scoped id-diff: 0 of 50 tracked ids survived, every cycle.
- **M3** (fully populated query tab: Data, Chart, Explain, Diagram, then close) — built correctly; closing left exactly 6 of 1915 subtree elements undisposed, the identical count and identical residual (the `LabelListItemRenderer`/`Text` finding above) as Phase 1's symlinked pass — not a new or worse defect, the published artefact behaves identically to the local build it was built from.

No new defects surfaced against the published package that were not already present and characterized against the symlink. The plan is complete.
