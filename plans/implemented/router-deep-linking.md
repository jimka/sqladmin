...
**`focusRecord`'s "not found" message needed a deferred `setTimeout`, not the plain `this.notify(...)` the `## Internal Structure` snippet shows.** Manual verification case 4 (`?rotated=true&record=999999` — case 6 under the URL scheme's original, pre-revision numbering) initially failed: the record view opened correctly, but the status line read `sqladmin · invoices: 3 rows` instead of `… no loaded record has key 999999`. Root cause: `AbstractStore.load()` (`packages/lib/src/typescript/lib/data/AbstractStore.ts`) emits `"load"` synchronously as the last statement before its own `async` body returns, so every `"load"` listener — including `focusRecord`'s `onLoad`, called via `this.notify(...)` — runs and returns *before* `load()`'s returned promise resolves. `SqlAdminController.openTable`'s `store.load().then(() => this.syncToPanel(id))` can therefore only run afterwards, as a later microtask, and `syncToPanel`'s `updateStatusFor` unconditionally overwrites the status bar with the row count — deterministically clobbering `focusRecord`'s message on every load, not just occasionally. Fixed by wrapping the `notify` call in `setTimeout(fn, 0)`: a macrotask runs only once the whole pending microtask queue (including `syncToPanel`'s chain, whatever its depth) has drained, so the "not found" message is reliably the one still showing once the tab settles. Verified live against the seeded demo database (`sales.invoices`, `?rotated=true&record=999999`) both before (message clobbered) and after (message survives) the fix. This is a defect in the plan's own `## Internal Structure` snippet for `focusRecord`, not a deviation in the surrounding design — `TableWorkPanel.ts`'s `focusRecord` method carries the fix and its reasoning in a comment.

**The URL scheme itself was reworked after this plan's first pass shipped, before merge.** A design review of the merged-but-unmerged-to-`main` branch asked for three changes to how a URL addresses an object, all applied in this revision: schema-scoped object routes (`table`/`view`/`matview`/`sequence`/`index`/`function`) now nest under `/schema/:schema/`, mirroring how the navigator itself contains things — a schema, expanded, is what reveals its objects — instead of the object's kind being the URL's first segment; a function's overload-disambiguating `signature` moved from a trailing path segment to a `?signature=` query parameter, collapsing the two registered function patterns into one; and role routes gained a three-way `/role/{user,group,predefined}/:role` bucket prefix mirroring `RolesTree`'s own Users/Groups/Predefined navigator sections, with the bucket deliberately left unvalidated against the role's real classification at consume time (the roles list has not loaded when `router.start()` applies the first route — the same limitation the `openReferenced*` Architecture Decision already documents, against a different tree). The schema's own `/schema/:schema/:view` route was already schema-first and needed no change. None of the router library's specificity logic changes: `RoutePattern.matchPattern` requires an exact segment count per pattern, so segment count alone disambiguates the nested forms, verified by reading `RoutePattern.ts` in the sibling `typescript-ui` repo rather than assumed (see the new "Schema-scoped objects nest under their schema in the path" Architecture Decision). `routeTargets.ts` gained `ROLE_BUCKETS`/`RoleBucket`, tested the same way `RELATION_KINDS` already was; `appRouter.ts`'s route table was rewritten in place (11 literal `register` calls now register 19 routes at run time, up from 12 registering 16, since the role loop replaces two literal registrations with a three-way loop of its own); no other file in the original "Files to Create/Modify/Delete" table needed a code change, since this revision is entirely about path *structure*, not the view-mode properties (`depth`/`rotated`/`record`) the untouched files already handle. The manual verification section was re-run live against the seeded demo database with the new URLs rather than mechanically re-pointed at the old case list — that pass also caught and corrected a stale claim in case 12 (now case 13): a table's fetch failure leaves its tab open with an empty grid rather than closing it. `openTable` still uses the same deferred `openAsyncPanel` pattern as every other tab, so the original explanation's *shape* was right; what was wrong was assuming its builder callback throws for a missing table. It does not — `fetchColumns`/`getTablePrivileges` resolve rather than reject for a nonexistent table (the backend's `columns` route returns an empty list, not a 404), so the deferred build succeeds and the tab opens; the 404 only arrives afterwards, from the separate `store.load()` call, caught by the store's own `"exception"` listener rather than the Dock's deferred-panel handler. Both mechanisms predate this plan and are unrelated to this revision; the live re-run is what surfaced the original wording's incorrect causal claim.
