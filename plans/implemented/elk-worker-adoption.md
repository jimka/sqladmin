...
- **Manual verification (checks 4, 5, 7) was actually performed, not just claimed, and here is the record an earlier audit round found missing** (a first attempt at recording check 4 relied on an INP trace of the triggering click, which a later audit round correctly rejected — elkjs's layout call is dispatched from an unawaited `.then()` after the click handler returns, per `relayout()` in the library's `DiagramView.ts`, so a fast click-processing time proves nothing about what runs afterward; superseded by the measurement below). Driven live via `chrome-devtools` against a `npm run dev` build (Host `localhost`, per this session's login setup) with Postgres seeded per `db/init/`:
  - **Check 4 (off-thread, no freeze)** — measured directly, not inferred from click timing: a `setInterval` heartbeat (20 ms period) was installed on the page before login and left running for the whole session (2,831 ticks total, ~57 s). It bracketed the right-click → "Show ▸ Schema diagram" interaction that opens the 154-table `hub` schema diagram — the click landed at heartbeat time 41,520 ms; the heartbeat was stopped at 57,283 ms, once the diagram's table labels were confirmed present in the DOM — so the ~15.7 s window from click to confirmed render (~780 of the 2,831 ticks) covers the whole layout. Filtered to that window, the **largest gap between two consecutive ticks was 178 ms**: since a `setInterval` callback is a macrotask, any main-thread block of duration *N* necessarily shows up as an inter-tick gap of at least *N*, so a 178 ms max gap is a direct upper bound on main-thread blocking during the interaction — nowhere close to a multi-second freeze, and consistent with ordinary browser jank rather than the main thread being blocked for the layout's duration. Had ELK's compute run on the main thread as before, this technique would have shown a single gap on the order of the old ~14 s freeze; it didn't. (What it does *not* by itself prove is that a Worker specifically was used, as opposed to some other reason the main thread stayed free — check 5's 200 on the worker asset is what establishes that a Worker was actually constructed.) The diagram rendered correctly (154 tables, confirmed via the "sqladmin · hub: diagram (154 tables)" status line, a full-fit screenshot, and a DOM check that both the alphabetically-first and last table names were present).
  - **Check 5 (worker asset resolves)** — `frontend/node_modules/elkjs/lib/elk-worker.min.js?worker_file&type=classic` appeared in the Network panel with a 200/304, and `npm run build` (production) emits it as `dist/assets/elk-worker.min-*.js`, confirming Vite resolves the bare specifier from the app's own `elkjs` in both dev and prod.
  - **Check 7 (other diagrams unaffected)** — also opened `DatabaseDiagramPanel`'s Overview mode (a different construction site — the internal-construction group, not the extends-`DiagramView` group check 4 exercised) against the same connection; it rendered its schema-overview graph correctly. Only this one of the plan's five listed diagram surfaces (line 127: relation, database, role-grants, role-membership, explain) was spot-checked beyond `SchemaDiagramPanel`; the other three are unexercised live (risk judged low — identical, mechanical wiring to the two that were checked).
  - Zero console errors or warnings were logged during either open beyond a pre-login 401 unrelated to diagrams. (An earlier note additionally claimed this silence specifically ruled out elkjs's `workerUrl`-unavailable warning — that reasoning was wrong and has been removed: the app passes `workerFactory`, never `workerUrl`, so that particular warning could never fire regardless of outcome, and the library's fallback path makes no `console.*` calls at all. The load-bearing evidence for a genuinely-constructed worker is check 5's 200 on the worker asset combined with check 4's heartbeat, not console silence.)
  - **Check 6 (CSP fallback smoke) was not performed.** The plan marks it "optional but recommended once." The fallback logic itself is covered by the upstream library's `ElkLayoutEngine.test.ts`, including the specific "workerFactory provided, factory throws (e.g. Worker undefined / CSP)" case; this check would only additionally confirm sqladmin's own CSP headers don't already block workers, which the app doesn't currently set (no CSP middleware anywhere in `backend/`), so there's nothing here to interact with a strict `worker-src 'none'` policy. Recorded as consciously skipped, not silently dropped.
- **`frontend/package.json:20`'s `"@jimka/typescript-ui": "^0.2.0"` does not yet resolve to a version containing `elkWorkerFactory`, and `package-lock.json` pins the real, published `0.2.0`.** Typecheck, the build, and the manual verification above all pass only because `frontend/node_modules/@jimka/typescript-ui` is a symlink currently pointed at the (unmerged, unpublished) `typescript-ui` `feature/elk-layout-web-worker` worktree — a plain `npm ci` against the registry (`npm view @jimka/typescript-ui versions` lists only `0.1.0, 0.1.1, 0.2.0`) fails typecheck on all six construction sites, because `package-lock.json` pins `0.2.0` by resolved URL and integrity hash, not the local worktree. `^0.2.0` also would not admit a `0.3.0` — the conventional bump level for an additive, backward-compatible option — once one is published. This mirrors the plan's own [Overview](#overview) ordering gate ("this feature typechecks and runs only after that library plan is implemented **and** `npm run build:lib` has been run") but the plan never mentions the *published-package* side of that gate. Not fixed in this branch — there is no real published version to bump to yet, and dependency-version bumps are a manual step the user runs by hand — but recorded as a discoverable, actionable item in [`TODO.md`](TODO.md) "Known issues / loose ends" (not just here, where nothing reads it post-merge) so the range gets bumped and the lockfile regenerated once typescript-ui publishes the release.
- **The audit/fix loop hit its 5-iteration cap with one BLOCKING finding still open, per the `audit` skill's exit condition** (an over-broad claim in `QueryPanel.ts`'s `diagramSlot` comment, introduced while narrowing an earlier round's "no dispose API at all" overclaim about the new Worker leak — see [`TODO.md`](TODO.md) "Known issues / loose ends"). The finding was a well-understood, low-risk wording fix (the comment implied `dispose()` would reclaim nothing at all, when it would actually free real, pre-existing theme-subscription leaks — just not the Worker specifically), so it was applied directly rather than left open, **without a further fresh-context re-audit**, since the loop had already reached its cap. Recorded here per the skill's instruction to surface a cap exit plainly rather than silently presenting the branch as fully audit-clean.
