...
- **Verification row 7 (`grep -rn 'measureTextWidths' src/` → "exactly 1 match, in `SqlAdminController.ts`") does not hold as literally stated.** The actual grep returns 3 matches: the one production call site in `SqlAdminController.ts`, plus two doc-comment mentions — the `MeasureWidths` JSDoc in `uniformNodeWidth.ts` and the `measureWidths` parameter's JSDoc in `buildSchemaDiagram.ts`. Both comments were written to match this plan's own text verbatim: the `MeasureWidths` doc comment is exactly the one given in [Public API](#public-api) (`` `Util.measureTextWidths` satisfies it; the diagram builders take it as a parameter... ``), and the `buildSchemaDiagram` parameter doc follows step 12's instruction to say "the app supplies `` `Util.measureTextWidths` ``" verbatim. Implementing those two instructions as written necessarily produces the extra grep hits the Verification row rules out — the plan's own Public API text and its row-7 expectation are mutually inconsistent, not something the code can satisfy simultaneously. The substantive intent of row 7 — that the measurer is only ever *supplied* (called) from one production site — holds: `Util.measureTextWidths` is passed as an argument exactly once, at `SqlAdminController.ts:1579`.

- **`absorbSlackIntoGreedy` visibly widens several grids past their content, beyond what a few of the manual-verify rows describe.** Walking [Expected Behaviour › grids](#manual-verify--grids) in the browser at 1500×800 turned up a real effect the plan's footnote [^cap] names but the per-row predictions understate: the library hands *any* leftover host width to *every* `string`/`auto` column that declares no `maxWidth` (`Table.absorbSlackIntoGreedy`), split equally among them, regardless of how narrow each one's own content-derived width already is. On the main data grid (`sales.invoices`, 3 rows), `net`/`vat`/`total` are `numeric` columns — `string`-typed on the wire — and are the only three columns with no `maxWidth`; each grew to roughly a third of the grid's leftover width (measured ~317px each) rather than staying content-narrow, because the id/document_no/order_id columns are `number`-typed and excluded from the split. The Indexes grid showed the same mechanism: `definition`'s content-derived width clamps at `AUTO_WIDTH_CAP_PX` (400px) as documented, but the post-clamp slack pass then adds back well over 280px to *both* `definition` and `name` (the grid's only two eligible columns), so `definition` renders past 680px and `name` — holding only `invoices_pkey` — renders past 350px. Neither is a defect: every column is still sized from real content sampling first, nothing clips, nothing collapses, and both effects reproduce for any grid with wide unused space and only one or two eligible flex columns (the six-column `sales.invoices` grid and the four-section Structure panel are exactly that shape; `wide.cols_60`'s sixty columns dilute the same slack across far more columns and so it isn't visible there). The rows this affects — "no column stretched to a viewport share it does not need" (main grid), "Definition is the wide column, at most 400px" (Indexes) — describe the content-derived floor correctly but not the slack pass on top of it. Filed as a `LIBRARY_NOTES.md`-style observation here rather than as its own entry there, since it is the same `absorbSlackIntoGreedy` mechanism footnote [^cap] already names as deliberate, not a new finding.

- **One sub-pixel label clip observed, unrelated to this plan's changes.** On the Foreign Keys grid, `name` renders `invoices_order_id_fk…` with a trailing ellipsis: the cell's own `scrollWidth` (155px) exceeds the derived column width by 4px. This is `Util.measureTextWidths`' own estimate falling a few pixels short of the string's real rendered width under the theme font — a pre-existing measurement-precision gap in the library's text measurer, not something `autoSizeColumns` introduces or that this plan's code touches.
