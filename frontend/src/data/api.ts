...
/**
 * Build a URL under the app's `/api` route from segments, percent-encoding
 * each one so a database/schema/relation/role name containing `/`, `#`, a
 * space, or any other reserved character stays inside its own path segment
 * instead of corrupting the route or silently truncating. Every request URL
 * in this file is built through this one function — grep for the literal
 * `/api` prefix under `frontend/src/data/` to confirm nothing else spells it
 * out. Route words are separate arguments, never folded into one string:
 * splitting `"ddl/table/create"` into three arguments keeps them as plain
 * path segments, while passing it as a single argument instead
 * percent-encodes its own slashes and 404s.
 *
 * @param segments - The route segments, in order. `undefined` (an absent
 *   optional `DbObjectRef` field) becomes an empty segment rather than the
 *   literal string "undefined".
 *
 * @returns The joined, percent-encoded path.
 */
...
export function login(details: LoginDetails): Promise<Session> {
    return postJson<Session>(apiPath("login"), details);
}
...
export async function whoami(): Promise<Session | null> {
    const response = await fetch(apiPath("whoami"));
...
export function runQuery(connectionId: string, sql: string): Promise<QueryResult> {
    return postJson<QueryResult>(apiPath(connectionId, "query"), { sql });
}
...
export function runExplain(
    ...
    return postJson<QueryExplainResult>(apiPath(connectionId, "explain"), {
        sql,
...
export function executeDdl(connectionId: string, sql: string): Promise<QueryStatusResult> {
    return postJson<QueryStatusResult>(apiPath(connectionId, "ddl", "execute"), { sql });
}
...
export function getRoles(connectionId: string): Promise<RoleSummary[]> {
    return getJson<RoleSummary[]>(apiPath(connectionId, "roles"));
}
...
export function getRoleDetail(connectionId: string, role: string): Promise<RoleDetail> {
    return getJson<RoleDetail>(apiPath(connectionId, "roles", role));
}
